...
public class RenderItemEntityExtended extends ItemEntityRenderer {
    ...
    public void render(ItemEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int packedLight) {
        ...
        int warningStartTicks = DespawnNotifierConfig.itemFlashStartTime.get() * 20; // 转换为ticks
        if (remainingTimeTick <= warningStartTicks) {
            float alpha;
            if (DespawnNotifierConfig.useBreathingEffect.get()) {
                // 呼吸效果（淡入淡出）
                alpha = calculateBreathingAlpha(remainingTimeTick, warningStartTicks);
            } else {
                // 传统闪烁效果
                alpha = calculateBlinkingAlpha(remainingTimeTick, warningStartTicks);
            }
            // 使用透明度渲染
     ...
     * 计算呼吸效果的透明度
     */
    private float calculateBreathingAlpha(int remainingTimeTick, int warningStartTicks) {
        float timeRatio = (float) remainingTimeTick / warningStartTicks; // 1.0 到 0.0
        if (DespawnNotifierConfig.isUrgentFlashEnabled.get()) {
            // 加速模式：3个阶段
            if (timeRatio > 0.66f) {
                // 第一阶段：慢呼吸 (1秒周期)
                float cycle = (remainingTimeTick % 20) / 20.0f;
                return 0.4f + 0.6f * (0.5f + 0.5f * Mth.sin(cycle * 2 * Mth.PI));
            } else if (timeRatio > 0.33f) {
                // 第二阶段：中等呼吸 (0.6秒周期)
                float cycle = (remainingTimeTick % 12) / 12.0f;
                return 0.3f + 0.7f * (0.5f + 0.5f * Mth.sin(cycle * 2 * Mth.PI));
            } else {
                // 第三阶段：快速呼吸 (0.15秒周期) + 渐变消失
                float cycle = (remainingTimeTick % 3) / 3.0f;
                float breathingAlpha = 0.2f + 0.8f * (0.5f + 0.5f * Mth.sin(cycle * 2 * Mth.PI));
                // 在最后阶段添加渐变消失效果
                return breathingAlpha * (timeRatio * 3); // timeRatio * 3
            }
        } else {
            // 统一呼吸模式
            float cycle = (remainingTimeTick % 20) / 20.0f;
     ...
     * 计算闪烁效果的透明度
     */
    private float calculateBlinkingAlpha(int remainingTimeTick, int warningStartTicks) {
        float timeRatio = (float) remainingTimeTick / warningStartTicks;

        if (DespawnNotifierConfig.isUrgentFlashEnabled.get()) {
            // 加速模式：闪烁频率逐渐加快
            if (timeRatio > 0.66f) {
                // 第一阶段：慢闪烁
                return (remainingTimeTick % 20 < 10) ? 1.0f : 0.3f;
            } else if (timeRatio > 0.33f) {
                // 第二阶段：中等闪烁
                return (remainingTimeTick % 12 < 6) ? 1.0f : 0.2f;
            } else {
                // 第三阶段：快速闪烁 + 渐变消失
                float blinkAlpha = (remainingTimeTick % 3 < 2) ? 1.0f : 0.1f;
                return blinkAlpha * (timeRatio * 3); // 渐变消失
            }
        } else {
            // 统一闪烁
            float blinkAlpha = (remainingTimeTick % 20 < 10) ? 1.0f : 0.3f;
